...
from hierarchical_nu.utils.roi import CircularROI, FullSkyROI, ROIList
...
```python
events = Events.from_ev_file(IC86_II, scramble_ra=True, scramble_mjd=True)
```

```python
np.log10(events.energies.to_value(u.GeV))
```
...
```python
obs_time = lt.lifetime_from_mjd(MJD_min, MJD_max)
obs_time
```

```python
obs_time[IC86_II].to(u.s)
```

```python
ps_events = Events.from_file("ps_2.1_10_events.h5")
```

```python
ps_bg_events = events.merge(ps_events)
```

```python
ps_bg_events.to_file("ps_2.1_bg_26_events.h5")
```
...
fit = StanFit(my_sources, IC86_II, ps_bg_events, obs_time, priors=priors)
```
